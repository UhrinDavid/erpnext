...
class XMLItemImportLog(Document):
	def validate(self):
	...
	def on_update(self):
		...
		if frappe.db.count("XML Item Import Log") > 100:
			logs_to_delete = frappe.get_all(
				"XML Item Import Log",
				fields=["name"],
				order_by="import_datetime asc",
				limit=frappe.db.count("XML Item Import Log") - 100
			)
			for log in logs_to_delete:
				frappe.delete_doc("XML Item Import Log", log.name, ignore_permissions=True)
