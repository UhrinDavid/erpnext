...
frappe.ui.form.on('XML Order Import Settings', {
	refresh: function(frm) {
		...
		frm.add_custom_button(__('View Import Logs'), function() {
			frappe.set_route("List", "XML Order Import Log");
		}, __('Actions'));
	},
...
function trigger_manual_import(frm) {
	frappe.confirm(
		__('Are you sure you want to trigger a manual order import?<br><br>This may take some time depending on the feed size.'),
		function() {
			// Show progress indicator
			frappe.show_alert({
				message: __('Starting order import...'),
				indicator: 'blue'
			});
			...
			frappe.call({
				...
				callback: function(r) {
					...
					if (r.message && r.message.success) {
						frappe.show_alert({
							message: __('Order import completed successfully'),
							indicator: 'green'
						});
						...
						frappe.show_alert({
							message: __('Order import failed'),
							indicator: 'red'
						});
